...
import "./Contacts.css";
import { contactInfo } from "../data/dataInfo";
			...
			<aside className="contact-card box-shadow-1">
				...
				<small className="social-media">
					{Object.entries(contactInfo.socialMedia).map((el) => (
						<a href={el[1].href} target="_blank" rel="noreferrer">
							{el[1].img}
						</a>
					))}
				</small>
			...
			<div className="my-container">
				...
				<form className="contact-form box-shadow-1" onSubmit={submit}>
					...
					<input
						...
						title="Introduzca un Email correcto"
						pattern="^(\w+[/./-]?){1,}@[a-z]+[/.]\w{2,}$"
						required
						value={form.email}
						onChange={formChange}
					...
					<input className="my-btn" type="submit" value="ENVIAR MENSAJE" />
				</form>
				{isModal && (
